...
| 098 | Capture TikTok Explore, Following, and Live discovery correctly | Sr Dev (GPT-5.6 Terra, High) | **Architecture-reviewed — ready on merged `main`**; durable Explore/Following observations are separate from ephemeral `/live` sightings, locator identity uses Live wire v2, and static bridge support is separate from the daemon's dynamic Level-2 gate |
