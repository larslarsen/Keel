...
| 094 | Nodes must be able to find each other, not only find content | Sr Dev | **Implemented** 2026-08-13 — rendezvous key derived from the protocol identity, published with the first announce and looked up on a timer; gated with announcing so Level 1 neither advertises nor searches. Verified publishing on a live DHT; **two-node connection still unverified** |
