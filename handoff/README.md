...
| 095 | The word-level bar should track live search progress, not a static ratio | Sr Dev | **Designed in conversation, not yet built** — amends WO-068's "not a search/fetch axis" boundary; numerator is token-intersection match count (reusing `resolveShardEntries`'s existing primitive, no titles), denominator stays WO-068's fetch; progress must be match-count/global-count, never buckets-fetched/buckets-planned; needs a new live-delivery mechanism since `PEER_SEARCH` replies once today; two open questions flagged unconfirmed in the ticket |
