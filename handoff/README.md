...
| 077 | Contribution-level changes must reconfigure the running swarm | Sr Dev (Claude Sonnet/Opus) | **Runtime transition done; Level-2 source policy superseded by 084** — gate-first node replacement and Level-1 boundary stand; WO-084 replaced the incorrect mirror-only Level 2 |
| 078 | Resolve the Level-1 outbound/privacy-contract contradiction | Sr Dev (Claude Sonnet/Opus) | **Done** 2026-08-11 — closed the standing `PRIVACY.md`/consent-copy contradiction ("asks the network for nothing") the WO-077 policy had already outgrown; new `TestLiveRecordWireShapeCarriesNoFunnelState` allow-lists the live wire payload |
...
| 082 | Reconcile the standing architecture and current work-order authority | Architect / Sr Dev | **Audit updated 2026-08-12; remains open** — shipped 080/081/084 state and disclosure reconciled; final close waits on 085/088/083 |
| 083 | Split the extension control plane at its existing responsibility boundaries | Sr Dev (Claude Opus) | **Ready after WO-088** — WO-080 and WO-085 prerequisites are done; module boundaries decided |
...
| 086 | Show Level-2 contribution impact without retaining request history | Sr Dev (Claude Sonnet) | **Ready now** — 084/085 prerequisites done; local aggregate dashboard; no query or peer histories |
