...
| 095 | Responsive streaming distributed search and UI | Sr Dev (Claude Opus) | **Done** 2026-08-13 — `peer_search:3` turns PEER_SEARCH into a job: validated, acknowledged promptly with `PEER_SEARCH_STARTED` (carrying frozen per-word targets), then progress/word-progress/result/terminal events on reserved `evt-` envelope ids scoped to the originating session. New `daemon/swarm/search.go` runs tokens concurrently with a per-response deadline each and no query-wide clock, unions candidates, resolves whole broad string prefixes before judging saturation, applies the local full-query matcher, and implements the stop matrix; peer selection shuffles and prefers unused peers. New `extension/background/search_sessions.js` routes events to the Port that claimed the search id and cancels orphans; new `extension/page/search_stream.js` owns the page state machine and paints from the daemon plan — the extension no longer tokenizes, colours or counts. Fixed a latent framing hazard: `bridge.WriteMessage` wrote its length prefix and payload separately, which concurrent emitters could interleave into permanent stream corruption. Go + race + 223/223 extension tests pass. **Two-machine live QA pending** (needs both machines on the new build). Originally — async session-scoped job; candidate shards are unioned, missing public titles arrive through broad string buckets, the daemon streams only locally verified full-query results; four-way peer work drives schematic token bars while distinct confirmed candidates drive live per-word target bars; target+saturation/resource-budget stopping and cancellation are specified |
