...
| 095 | Responsive streaming distributed search and UI | Sr Dev (Claude Opus) | **Implemented 2026-08-13; WO-101 correction required before merge/live QA** — `peer_search:3`, session routing, parallel token work, daemon render plan and framing repair are present; WO-099/100 fixed the first two review rounds, and WO-101 closes the remaining termination boundaries |
...
| 099 | Close streaming-search lifecycle and resource-accounting gaps | Sr Dev (Claude Opus) | **Implemented** 2026-08-13 — page/session isolation, early-event preservation and downgrade cancellation landed; WO-100 fixed the hard-reader ceiling, typed prefix results, completion memoization, shared-candidate barrier and identity retirement; WO-101 is the remaining merge gate |
| 100 | Finish search-budget and resolution atomicity | Sr Dev (Claude Opus) | **Implemented** 2026-08-13 — architecture review accepted the main corrections and opened WO-101 for reservation-versus-spend accounting, unresolved saturation, paged-error classification and global stop-all retirement |
| 101 | Close distributed-search termination semantics | Sr Dev (Claude Opus) | **Ready — merge gate** for the stacked WO-095/097/099/100 branch; no query, UI, privacy, entitlement or key-scheme redesign |
| 101 | Close distributed-search termination semantics | Sr Dev (Claude Opus) | **Done** 2026-08-13 — review follow-up to WO-100 at `a59953b`; all four gaps real. The byte meter now separates *leased* from *spent* (`committed + reserved <= limit`): a read finding every remaining byte reserved waits for a settlement instead of latching exhaustion and cancelling the job while allowance was about to be refunded. Candidates carry a published disposition — checked, absent, or unresolved — through the completion barrier, so a response that gained nothing because a peer failed no longer advances a word\'s saturation streak and cannot stop a search below its retained target. Paged transport failures stay typed across the boundary: pre-response failures are unavailable, framing/authentication failures are invalid, and the budget sentinel is neither. `stopAll` stops jobs without deregistering them, so cancelled work keeps its slot until it retires. Go + race + 226/226 extension tests pass. **Branch is the merge candidate for the whole search stack; two-machine live QA still pending** |
