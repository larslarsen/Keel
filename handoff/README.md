...
| 095 | Responsive streaming distributed search and UI | Sr Dev (Claude Opus) | **Implemented 2026-08-13; WO-099 correction required before acceptance/live QA** — `peer_search:3`, session routing, parallel token work, daemon render plan and framing repair are present; review found acknowledgement/event state loss, downgrade terminal reporting, catalogue concurrency/byte-accounting, transient-resolution retry and diagnostic-identifier gaps. Existing Go + extension tests pass but do not cover those boundaries |
...
| 098 | Capture TikTok Explore, Following, and Live discovery correctly | Sr Dev (Claude Opus) | **Ready — independent of WO-095**; preserves distinct feed surfaces, sends Explore/Following video cards through ordinary observations, sends `/live` cards through a dedicated Live-sighting path keyed by canonical `@creator/live` locator, repairs dropped TikTok platform and idle live-room bugs, and populates/gossips Keel Live at Level 2+ without fake room ids or APIs |
| 099 | Close streaming-search lifecycle and resource-accounting gaps | Sr Dev (Claude Opus) | **Implemented 2026-08-13; WO-100 correction required before acceptance/live QA** — page/session isolation, early-event preservation and downgrade cancellation landed; review found the hard meter still charges after whole reads, successful prefix coalescing is not retained, concurrent resolution can advance saturation early, and retiring ids can race reuse |
| 100 | Finish search-budget and resolution atomicity | Sr Dev (Claude Opus) | **Ready — required before WO-095/099 live QA**; meter bytes during reads with a truthful budget terminal, preserve catalogue complete/incomplete outcomes, memoize successful job-local prefix completion, join concurrent candidate resolution before saturation, and retire jobs by object identity |
| 098 | TikTok Explore, Following and Live discovery | Sr Dev | **Open** — supersedes WO-076 |
| 099 | Close streaming-search lifecycle and resource-accounting gaps | Sr Dev (Claude Opus) | **Done** 2026-08-13 — review follow-up to WO-095 at `4455f76`, and every gap it named was real. Jobs are per-page again (`startJob` had been cancelling every job on the native session, so a search in one tab killed the one in another); a node-wide `searchSem` and a named `maxActiveSearchJobs` ceiling with a `search_busy` refusal replace displacement as the bound. `PEER_SEARCH_STARTED` now annotates existing render state via `applyTargets` instead of re-rendering and discarding counts, phases and streamed results. A contribution downgrade, consent withdrawal or shutdown cancels promptly through `stopDistributedSearches` and terminates as `PEER_SEARCH_CANCELLED` with a bounded reason, never `COMPLETE`. The aggregate budget is metered inside `requestPaged`, so catalogue buckets and rejected/partial reads are charged too, and exhaustion cancels outstanding reads; the four-response permit is held through candidate resolution and prefix traversals coalesce job-wide. Candidates gained a retryable state so one transient catalogue failure no longer discards a video permanently. Search diagnostics are identifier-free, and `search_id` must be a canonical UUID and unique among live jobs. Go + race + 226/226 extension tests pass. **WO-095's two-machine live QA still pending** |
| 100 | Finish search-budget and resolution atomicity | Sr Dev (Claude Opus) | **Done** 2026-08-13 — review follow-up to WO-099 at `f45510c`; all five gaps real and all in freshly landed concurrency code. The byte ceiling moved *inside* the reader (`budgetReader` reserves from one job-wide balance before each read and refunds the remainder), so four concurrent readers share a real ceiling and no response can overshoot by the gap between the 8 MiB floor and the 64 MiB transport cap; exhaustion resets outstanding streams and `outcome()` reads it from the meter so a budget spent in the last provider response still reports `budget` rather than `exhausted`. Catalogue traversals return a typed `catalogueResult` distinguishing complete-empty, complete-with-rows, authenticated-incomplete, unavailable and invalid — falling through every provider is no longer a fabricated empty success. `prefixGroup` memoizes verified complete traversals for the job while leaving weaker outcomes retryable, and wakes waiters on cancellation. A `candidateState` barrier stops a response recording itself gainless while another worker is still resolving its candidate. Jobs retire by identity, so a reused UUID cannot be deleted by the previous job\'s deferred finish. Go + race + 226/226 extension tests pass. **WO-095\'s two-machine live QA still pending** |
