...
| 095 | Responsive streaming distributed search and UI | Sr Dev (Claude Opus) | **Implemented 2026-08-13; WO-102 correction required before merge/live QA** — `peer_search:3`, session routing, parallel token work, daemon render plan and framing repair are present; WO-099/100/101 fixed the first three review rounds, and WO-102 closes the remaining stop-cause boundaries |
...
| 099 | Close streaming-search lifecycle and resource-accounting gaps | Sr Dev (Claude Opus) | **Implemented** 2026-08-13 — page/session isolation, early-event preservation and downgrade cancellation landed; WO-100/101 fixed the hard-reader ceiling, typed prefix results, completion memoization, shared-candidate barrier, reservation accounting and identity retirement; WO-102 is the remaining merge gate |
| 100 | Finish search-budget and resolution atomicity | Sr Dev (Claude Opus) | **Implemented** 2026-08-13 — its main corrections landed; WO-101 fixed reservation-versus-spend accounting, unresolved saturation and global stop-all retirement, while WO-102 closes final cause propagation |
| 101 | Close distributed-search termination semantics | Sr Dev (Claude Opus) | **Implemented** 2026-08-13 — lease/spend accounting, candidate dispositions and exact stop-all retirement landed; review opened WO-102 because budget is still swallowed as unavailable, local title-read failure can prove false absence, and cancelled waiters may lease refunded bytes |
| 102 | Preserve distributed-search stop causes end to end | Sr Dev (Claude Opus) | **Ready — final merge gate**; preserve budget through the full catalogue path, keep local resolution failures unresolved, and make cancellation win before a meter waiter leases more capacity |
| 102 | Preserve distributed-search stop causes end to end | Sr Dev (Claude Opus) | **Done** 2026-08-13 — review follow-up to WO-101 at `db31531`; all three gaps real. `ErrSearchBudget` is no longer classified as a provider outcome at all: it short-circuits the catalogue page fetch, provider discovery and the known-peer fallback (which now also checks the job context before and during its walk), so a terminated job stops instead of walking the peer list reporting itself as a provider failure. A local planning or title-read failure can no longer prove a candidate absent — `resolveTitles` settles nothing on local error, so the candidate stays retryable and the response reports unresolved. `budgetMeter.reserve` checks cancellation before capacity, so a waiter woken by a refund cannot lease it for a job that was already withdrawn. The malformed-vs-silent distinction is now driven through the real transport rather than by calling the classifier. Go + race + 226/226 extension tests pass; `git diff --check main...HEAD` clean. **Stack is the merge candidate; two-machine live QA still pending** |
