...
| 099 | Close streaming-search lifecycle and resource-accounting gaps | Sr Dev (Claude Opus) | **Ready — required before WO-095 live QA**; allow bounded independent page jobs on one native session, preserve early events when targets arrive, make downgrades cancel honestly, include catalogue work in concurrency/byte budgets, retry transiently unresolved candidates, sanitize search-path logs, and validate job ids |
