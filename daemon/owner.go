...
//
// Restored after being reverted (f2b8ce0) with no recorded reason, then
// rediscovered live via an adversarial re-read of this file: nothing else in
// the codebase ever grew an EOF/closed-pipe carve-out, so every
// install-triggered owner shutdown still hits this race. The overlapped-I/O
// pipe fix that followed the revert (9c786ce/1bc9c8a) fixes a different bug —
// why a read failed at all on a synchronous handle — not this one, which is
// about a dial legitimately succeeding against an owner that is already
// closing. Both are real; neither substitutes for the other.
...
func connectOrSpawnOwner(p ownerPaths, secret string) (net.Conn, error) {
	...
	if err == nil {
		...
		if hsErr != nil {
			...
			// that is closing under it. That is not an incompatible owner, it
			// is no owner — so start one, rather than failing the browser with
			// a transport error it can do nothing about. Every install stops
			// the previous owner, so this window is hit routinely.
	...
	for time.Now().Before(deadline) {
		...
		if lastErr == nil {
			...
			_ = conn.Close()
			// Same race, on the way up: a contender's owner can win the
			// endpoint and then exit, so the handshake meets a pipe closing
			// under it. Keep waiting rather than failing the browser.
			if !isOwnerGoneError(hsErr) {
				return nil, hsErr
			}
			lastErr = hsErr
